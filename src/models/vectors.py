...
class RecurrentMixturev2(tkm.Model):
    ...
    def __init__(self, T, recurrent_cell_args, **kwargs):
        super(RecurrentMixturev2, self).__init__()
...
class XMixture(tkm.Model):
    def __init__(self, P, hidden_layers, uniform_init, T=1.0, batchnorm=False, units=128, use_kernel=True, use_bias=True, **kwargs):
        super(XMixture, self).__init__()
        ...
        self.final_dense = MaskedDense(units=P,
    ...
    def call(self, x, training=None, **kwargs):
        ...
        for i in range(len(self.denses)):
            ...
            if self.batchnorm:
                x_enc = self.bns[i](x_enc, training=training)
...
class NetL1BallVector(tkm.Model):
    def __init__(self, n_permutations, temperature=1.0):
        super(NetL1BallVector, self).__init__()
...
class L1BallVector(tkm.Model):
    def __init__(self, n_permutations, temperature=1.0, name='l1ballvector/mixture', **kwargs):
        super(L1BallVector, self).__init__()
...
class VectorList(tkm.Model):
    def __init__(self, n_permutations_list, name='vectorlist', **kwargs):
        super(VectorList, self).__init__()
        self.vectors = []
...
class L1BallVectorList(tkm.Model):
    def __init__(self, n_perms, temperature=1.0, **kwargs):
        super(L1BallVectorList, self).__init__()
        self.vectors = []
