...
from models.layers.permutation_layers import PermutationDense, PermutationDensev2, PermutationDenseCategorical
...
class PermutationIm2Seq(tkm.Model):
    ...
    def call(self, inputs, training=None, y=None, **kwargs):
        ...
        if training:
            ...
            key = 'loss'
            outputs_dict[key] = dict()
            outputs_dict[key]['mixture_logits'] = tf.tile(tf.expand_dims(permutation_mixture_logits,
                                                                         axis=0),
                                                          (batch_size, 1))
            outputs_dict[key]['mixture'] = tf.tile(tf.expand_dims(permutation_mixture,
                                                                         axis=0),
                                                          (batch_size, 1))
            outputs_dict[key]['prediction_logits'] = logits_in_order
            ...
            outputs_dict[key]['mixture'] = tf.tile(tf.expand_dims(permutation_mixture,
                                                                         axis=0),
                                                          (batch_size, 1))
            outputs_dict[key]['prediction_logits'] = mean_logits_in_order
            ...
            output = tf.math.reduce_sum(mean_outputs_in_order * permutation_mixture_expended,
                                        axis=1)
            outputs_dict['global_pred'] = output
        return outputs_dict
...
class Maonet(tkm.Model):
    def __init__(self,
        ...
        self.H_ctrl = recurrent_mixture.get_units()
        print('H : ', self.H_ctrl)

        self.compresser_pred = tkl.Dense(units=self.H_pred,
                                         activation='relu')
        self.compresser_ctrl = tkl.Dense(units=self.H_ctrl,
                                         activation='relu')
    ...
    def call(self, inputs, training=None, y=None, **kwargs):
        ...
        # (B, H_pred)
        inputs_pred = self.compresser_pred(inputs, training=training)
        # (B, 1, H_ctrl)
        inputs_ctrl = self.compresser_ctrl(inputs, training=training)

        # (B, 1, H_pred)
        inputs_pred = tf.expand_dims(inputs_pred, axis=1)
        # (B, 1, H_ctrl)
        inputs_ctrl = tf.expand_dims(inputs_ctrl, axis=1)

        # (B, 1, H)
        state_mixture = inputs_ctrl
        state_main = inputs_pred

        if not self.mixtX:
            state_mixture = tf.zeros_like(state_mixture)
        ...
        for k in range(self.T):
            ...
            # output_mixture: (B, T!/(T - k)!, H_ctrl), states_mixture: (B, T!/(T - k)!, H_ctrl)
            ...
            state_mixture = tf.reshape(tf.transpose(state_mixture, (0, 2, 1, 3)),
                                       (batch_size, cur_leaf * branches, self.H_ctrl))
            ...
            state_main = tf.reshape(tf.transpose(state_main, (0, 2, 1, 3)),
                                    (batch_size, cur_leaf * branches, self.H_pred))
...
class DropoutMaonet(tkm.Model):
    def __init__(self,
        ...
        # Handling input compression
        self.H_pred = recurrent_cell_args['units']
        self.H_ctrl = self.recurrent_mixture.get_units()
        
        self.compresser_pred = tkl.Dense(units=self.H_pred,
                                         activation='relu')
        # self.pred_bn = tkl.BatchNormalization()


        self.compresser_ctrl = tkl.Dense(units=self.H_ctrl,
                                         activation='relu')
        # self.ctrl_bn = tkl.BatchNormalization()


    def call(self, inputs, input_transmission=None, training=None, y=None, **kwargs):
        """
        inputs: float (B, I).
        y: float (B, T).
        """
        ...
        # (B, 1, H_pred)
        inputs_pred = self.compresser_pred(inputs, training=training)
        # inputs_pred = self.pred_bn(inputs_pred, training=training)
        inputs_pred = tf.expand_dims(inputs_pred, axis=1)

        # (B, 1, H_ctrl)
        inputs_ctrl = self.compresser_ctrl(inputs, training=training)
        # inputs_ctrl = self.ctrl_bn(inputs_ctrl, training=training)
        inputs_ctrl = tf.expand_dims(inputs_ctrl, axis=1)
        ...
        for k in range(self.T):
            ...
            if training:
                # DROPOUT STRATEGY:
                ## Remove Tasks that have already been processed.
                logit_mixture = tf.boolean_mask(logit_mixture, mixture_mask)
                logit_mixture = tf.reshape(logit_mixture,
                                           (batch_size, cur_leaf, self.T - k))

                ## Choose which permutation to keep. 
                # (B * cur_leaf, dropout, 1)
                kept_permutation = tf.expand_dims(sample_without_replacement(tf.zeros_like(logit_mixture), branches),
                                                  axis=-1)
                ...
                ## Filter the logits to get the kept logit only
                # (B, cur_leaf)
                ...
                # (B, cur_leaf, T - K)
                masked_logit_mixture = tf.boolean_mask(logit_mixture, mixture_mask)

                
                masked_logit_mixture = tf.reshape(masked_logit_mixture,
                                                  (batch_size, cur_leaf, self.T - k))

                float_mask = tf.dtypes.cast(mixture_mask, dtype=tf.float32)
                # tf.print('TIMESTEP : ', k)
                # tf.print('logit_max : ', tf.math.reduce_max(masked_logit_mixture))
                # tf.print('logit_min : ', tf.math.reduce_min(masked_logit_mixture))
                # tf.print('\n')

                masked_exp_logit = tf.math.exp(logit_mixture) * float_mask
            ...
            # (B, 1, cur_leaf, H_ctrl)
            state_mixture = tf.expand_dims(state_mixture, axis=1)
            # (B, branches, cur_leaf, H_ctrl)
            state_mixture = tf.tile(state_mixture, (1, branches, 1, 1))
            # (B, cur_leaf * branches, H_ctrl)
            state_mixture = tf.reshape(tf.transpose(state_mixture, (0, 2, 1, 3)),
                                       (batch_size, cur_leaf * branches, self.H_ctrl))
            ...
            if training:
                ...
                # TEST SAMPLING
                # masked_logit_mixture are of size (B, cur_leaf, T - k)
                # (B, cur_leaf, T)
                task_indices = tf.tile(tf.reshape(tf.range(self.T), (1, 1, self.T)),
                                       (batch_size, cur_leaf, 1))
                # (B, cur_leaf, T - k)
                task_indices_masked = tf.reshape(tf.boolean_mask(task_indices, mixture_mask),
                                                 (batch_size, cur_leaf, self.T - k))
                # (B * cur_leaf, branches)
                ## permutation in term of masked indices
                masked_logit_mixture = tf.reshape(masked_logit_mixture, (batch_size * cur_leaf, self.T - k))
                permutation_masked = tf.reshape(tf.random.categorical(masked_logit_mixture, branches,
                                                                      dtype=tf.int32),
                                                (batch_size, cur_leaf, branches, 1))


                ## Convert permutation in term of masked indices into permutation in term of task indices :
                
                # (B, cur_leaf) and (B, cur_leaf)
                perm_index, batch_index = tf.meshgrid(tf.range(cur_leaf), tf.range(batch_size))

                # (B, cur_leaf, branches, 1)
                batch_index = tf.expand_dims(tf.tile(tf.expand_dims(batch_index, axis=-1), (1, 1, branches)),
                                                     axis=-1)
                # (B, cur_leaf, branches, 1)
                perm_index = tf.expand_dims(tf.tile(tf.expand_dims(perm_index, axis=-1), (1, 1, branches)),
                                            axis=-1)

                # (B, cur_leaf, branches)
                permutation = tf.gather_nd(params=task_indices_masked,
                                           indices=tf.concat([batch_index, perm_index, permutation_masked],
                                                             axis=-1))
            ...
            if training:
                # Maps batch index with the task that it treats.
                # (B, T!/(T - (k + 1))!)
                ...
                # Select the task that is treated
        ...
        if training:
            ...
            mixtures = tf.concat(mixtures, axis=-1)
            outputs_dict['permutation_matrix'] = mixtures
            outputs_dict['permutation_matrices'] = dict()
            outputs_dict['permutation_matrices']['matrix'] = mixtures
            outputs_dict['permutation_matrices']['input'] = input_transmission
...
class Damonet(tkm.Model):
    def __init__(self,
        ...
        self.routing_dense1 = PermutationDensev2(n_task=self.T + 1,
                                                 units=routing_units,
                                                 activation='relu')
        self.routing_dense2 = PermutationDensev2(n_task=self.T + 1,
                                                 units=self.T,
                                                 activation='linear')
    ...
    def call(self, inputs, input_transmission=None, training=None, y=None, **kwargs):
        ...
        # (B, H)
        if self.input_compression:
            states_t = self.input_compresser(inputs, training=training)
        else:
            states_t = inputs
        ...
        # Initialization (t = 0)
        # (B, 1, H)
        states_t = tf.expand_dims(states_t, axis=1)
        # (B, 1, T)
        ...
        for t in range(self.T + 1):
            input_t = self.label_encoder(y_t, training=training)

            # (B, leaf_t, H), (B, leaf_t, H) 
            output_t, states_t = self.recurrent_cell(inputs=input_t,
                                                     states=states_t,
                                                     permutation=tasks_t,
                                                     training=training)
            ...
            if t > 0:
                ...
                output_pred_t = self.prediction_dense1(output_t,
                                                       permutation=tasks_t,
                                                       training=training)
            ...
            if t < self.T:
                ...
                # (B, leaf_t, T)
                output_mixture_t = self.routing_dense1(output_t,
                                                       permutation=tasks_t,
                                                       training=training)
                logit_t = self.routing_dense2(output_mixture_t,
                                              permutation=tasks_t,
                                              training=training)
                """
                # Permutation matrix: column_t
                # (B, leaf_t, T)
                exp_logit_t = tf.math.exp(logit_t) * tf.dtypes.cast(mask_t, tf.float32)
                # (B, leaf_t, T)
                column_t = exp_logit_t / tf.tile(tf.expand_dims(tf.math.reduce_sum(exp_logit_t, axis=-1),
                                                                axis=-1),
                                                 (1, 1, self.T))
                # (B, T)
                column_t = tf.math.reduce_mean(column_t, axis=1)
                columns.append(tf.expand_dims(column_t, axis=-1))
                """

                # Masking the tasks that have already been processed
                logits_t_masked = tf.reshape(tf.boolean_mask(logit_t, mask_t),
                ...
                if training:
                    ...
                    kept_permutations = tf.expand_dims(sample_without_replacement(tf.ones_like(logits_t_masked),
                                                                                  branches),
                                                       axis=-1)
                else:
                    # INFERENCE : Sampling of branches tasks based on the logit mixture coefficients
                    # (B, leaf_t, branches, 1) : kept task in term of masked indices
                    kept_permutations = tf.reshape(tf.random.categorical(tf.reshape(logits_t_masked,
                                                                                    (B * leaf_t, self.T - n_processed_t)),
                                                                         branches,
                                                                         dtype=tf.int32),
                                                   (B, leaf_t, branches, 1))
                ...
                # (B, leaf_t, branches)
                logits_t = tf.gather_nd(params=logits_t_masked,
                                        indices=kept_permutations)

                logits_logsumexp = tf.tile(tf.expand_dims(tf.math.reduce_logsumexp(logits_t, axis=-1), -1),
                                           (1, 1, branches))


                # Build column of logit matrices:
                # (B, leaf_t * branches)
                logits_t = tf.reshape(logits_t, (B, leaf_t * branches))
                logits_logsumexp = tf.reshape(logits_logsumexp, (B, leaf_t * branches))

                # (B, 1, leaf_t * branches)
                logits_t = tf.expand_dims(logits_t, axis=1)
                logits_logsumexp = tf.expand_dims(logits_logsumexp, axis=1)

                # (B, mult_to_leafs, leaf_t * branches)
                logits_t = tf.tile(logits_t, (1, mult_to_leafs, 1))
                logits_logsumexp = tf.tile(logits_logsumexp, (1, mult_to_leafs, 1))

                # (B, total_leafs, 1) 
                logits_t = tf.reshape(tf.transpose(logits_t, (0, 2, 1)),
                                      (B, total_leafs, 1))
                logits_logsumexp = tf.reshape(tf.transpose(logits_logsumexp, (0, 2, 1)),
                                              (B, total_leafs, 1))

                logits_mixture.append(logits_t)
                logsumexps.append(logits_logsumexp)
                ...
                states_t = tf.expand_dims(states_t, axis=1)
                # (B, branches, leaf_t, H)
                states_t = tf.tile(states_t, (1, branches, 1, 1))
                # (B, branches * leaf_t, H)
                states_t = tf.reshape(tf.transpose(states_t, (0, 2, 1, 3)),
                                      (B, leaf_t * branches, self.H))

                tasks_t = tf.reshape(tasks_t, (B, leaf_t * branches))   
                leaf_t = leaf_t * branches

        logits_mixture = tf.concat(logits_mixture, axis=-1)
        logsumexps = tf.concat(logsumexps, axis=-1)
        # (B, total_leafs, T)
        ...
        permutation_matrix = tf.math.reduce_mean(tf.one_hot(permutation_matrix, axis=-1, depth=self.T),
                                                 axis=1)
        mixture = tf.math.exp(logits_mixture - logsumexps)

        if training:
            outputs_dict['loss'] = dict()
            outputs_dict['loss']['log_mixture'] = logits_mixture - logsumexps
            ...
            outputs_dict['permutation_matrix'] = dict()
            outputs_dict['permutation_matrix']['matrix'] = permutation_matrix
            outputs_dict['permutation_matrix']['input'] = input_transmission
