...
def scaled_dot_product_attention(q, k, v, mask, temperature=1.0, epsilon=1e-6):
    ...
    """
    # tf.print("QUERY MEAN : ", tf.math.reduce_mean(q, axis=-1))
    # tf.print("QUERY STD : ", tf.math.reduce_std(q, axis=-1))

    # tf.print("KEY MEAN : ", tf.math.reduce_mean(k, axis=-1))
    # tf.print("KEY STD : ", tf.math.reduce_std(k, axis=-1))
...
class MultiHeadAttention(tkl.Layer):
    ...
    def _build_from_signature(self, input_shape):
