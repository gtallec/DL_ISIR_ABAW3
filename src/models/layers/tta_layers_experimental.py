...
from models.layers.ca_layers_experimental import CALayer
from models.layers.sa_layers_experimental import SALayer
...
class TTALayer(tkl.Layer):
    ...
    def call(self, qk, mask=None, training=None):
        query, key = qk
        query, att_tt = self.sa_tt(x=query,
                                   mask=mask,
                                   training=training)
        query, att_tx = self.ca_tx(qk=(query, key),
                                   mask=mask,
                                   training=training)
        return query, att_tt, att_tx

    def build(self, input_shape):
        print('TTA_LAYER build : ', input_shape)
...
class TTALayers(tkl.Layer):
    ...
    def call(self, qk, mask=None, training=None, **kwargs):
        query, key = qk
        atts_tt = []
        atts_tx = []
        for i in range(self.num_layers):
            query, att_tt, att_tx = self.tta_layers[i](qk=(query, key),
                                                       mask=mask,
                                                       training=training)
            qk = (query, key)
            atts_tt.append(att_tt)
            atts_tx.append(att_tx)
        return query, atts_tt, atts_tx
