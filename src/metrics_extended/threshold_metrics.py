...
class ClassificationScores(tf.keras.metrics.Metric):
    def __init__(self, metric_names, thresholds, pred_in, dataset_columns, **kwargs): 
        ...
        super(ClassificationScores, self).__init__(**kwargs)
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        # (M, N, T)
        self.tp.assign_add(tf.math.reduce_sum(y_pred_bin * y_true_bin, axis=0))

        # (M, N, T)
        self.tn.assign_add(tf.math.reduce_sum((1 - y_pred_bin) * (1 - y_true_bin), axis=0))

        # (M, N, T)
        self.fp.assign_add(tf.math.reduce_sum(y_pred_bin * (1 - y_true_bin), axis=0))

        # (M, N, T)
        self.fn.assign_add(tf.math.reduce_sum((1 - y_pred_bin) * y_true_bin, axis=0))
...
class TrainClassificationScores(ClassificationScores):
    def __init__(self, metric_names, threshold_step, n_coords, pred_in, dataset_columns, log_folder, **kwargs):
        super(TrainClassificationScores, self).__init__(metric_names=metric_names,
...
class TestClassificationScores(ClassificationScores):
    def __init__(self, metric_names, pred_in, dataset_columns, thresholds=None, **kwargs):
        ...
        super(TestClassificationScores, self).__init__(metric_names=metric_names,
...
class ABAW3ClassificationScores(TestClassificationScores):
    def __init__(self, metric_names, pred_in, dataset_columns, **kwargs):
        super(ABAW3ClassificationScores, self).__init__(metric_names=metric_names,
                                                        pred_in=pred_in,
                                                        dataset_columns=dataset_columns)
...
SUPPORTED_THRESHOLD_METRICS = {"th_train": TrainClassificationScores,
                               "th_test": TestClassificationScores,
                               "th_abaw3": ABAW3ClassificationScores}
